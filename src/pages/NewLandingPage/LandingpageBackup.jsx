...
import lpstyle from "../NewLandingPage/LandingPageBackupcss.module.css";
...
function LandingPage() {
  ...
  const [combinedClasses, setCombinedClasses] = useState(`${lpstyle.list}`);
  const [navLogo, setNavLogo] = useState(true);

  const smallNav = () => {
    // Check if "smalllist" class is present in the current state
    if (combinedClasses.includes(`${lpstyle.smalllist}`)) {
      setNavLogo(false);

      // If present, remove "smalllist"
      setCombinedClasses((prevClasses) =>
        prevClasses.replace(`${lpstyle.smalllist}`, " ")
      );
    } else {
      // If not present, add "smalllist"
      setCombinedClasses((prevClasses) => prevClasses + `${lpstyle.smalllist}`);
      setNavLogo(true);
    }
  };
  ...
  return (
    <>
      <div id="landing-page1" className={lpstyle.landingPage1}>
        <div className={lpstyle.navbar}>
          ...
          <div
            className={lpstyle.testlogo}
            style={{
              display: navLogo ? `${lpstyle.block}` : `${lpstyle.none}`,
            }}
          >
            <img
              className={lpstyle.o5}
              style={{ cursor: "pointer" }}
              src={o4}
              alt=""
              onClick={smallNav}
        ...
        <div className={lpstyle.mobileNavbar}>
          <div>
            <img src={logo} alt="Choira Logo" style={{ cursor: "pointer" }} />
          </div>
          <div>
        ...
        {/* sidebar-----------=-- */}
        <div
          className={`${lpstyle.lpSidebar} ${
            sidebarVisible ? `${lpstyle.lpSidebarAfter}` : ""
          }`}
        >
